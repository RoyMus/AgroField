...
const SheetDataEditor = ({ sheetData, onSaveProgress, onSaveToNewSheet }: SheetDataEditorProps) => {
  for (let i = 0; i < sheetData.values.length; i++) {
    if (sheetData.values[i][0] != null && sheetData.values[i][0].getValue().trim() != "")
    {
  ...
  for (let i = found_headers_row_index + 1; i < sheetData.values.length; i++) {
    ...
    {
      if (sheetData.values[i][j] != null && sheetData.values[i][j].getValue().trim() != "")
      {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < topBarRow.length; i++) {
      if (topBarRow[i].getValue() != "") {
        topBarIndex = i;
        break;
      }
    }
    for (let i = 0; i < faucetRow.length; i++) {
      if (faucetRow[i].getValue() != "") {
        faucetIndex = i;
        break;
      }
    }
    sheetData.values[faucetRowIndex][faucetIndex].originalValue = `${faucetConductivity} - מוליכות ברז`;
    if (isTemplate) {
      sheetData.values[topBarRowIndex][topBarIndex].originalValue = `${place} - ${plant} - ${grower}`;
    }
  ...
  const calcAverages = () =>
  {
    for (let i = minColIndex; i <= maxColIndex; i++)
    {
      ...
      {
        temp = parseFloat(sheetData.values[j][i].getValue());
      ...
      sum /= counter;
      sheetData.values[dataRows.length - 1][i].newValue = `${sum}`;
      const wantedResult = sheetData.values[dataRows.length - 2][i]?.getValue().split('-');
      let isBetween = false;
      ...
      sheetData.values[dataRows.length - 1][i].formatting = {
  ...
  useEffect(() => {
    ...
    speak(headers[currentColumnIndex].getValue());
    setCurrentValue("");
  ...
  const recordCurrentValue = async () => {
    ...
    sheetData.values[currentRowIndex][currentColumnIndex].newValue = currentValue;
    toast({
  ...
  const resetCurrentCell = () => {
    ...
    sheetData.values[currentRowIndex][currentColumnIndex].newValue = "";
  };
