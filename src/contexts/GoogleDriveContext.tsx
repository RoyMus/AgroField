...
export const GoogleDriveProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [pendingRefetch, setPendingRefetch] = useState<{ fileId: string; sheetName?: string } | null>(null);
  ...
  // Re-fetch sheet from Google Drive on page refresh (replaces localStorage cache)
  useEffect(() => {
    if (pendingRefetch && isAuthenticated && accessToken) {
      const { fileId, sheetName } = pendingRefetch;
      setPendingRefetch(null);
      readSheet(fileId, sheetName);
    }
  }, [isAuthenticated, pendingRefetch]);
  ...
  const loadSheetByName = async (sheetName: string) => {
    ...
    setSheetData(allSheets[sheetName]);
  };
  ...
  const createNewSheet = async (fileName: string): Promise<{ success: boolean; url?: string; error?: string }> => {
    ...
    if (!allSheets) return { success: false, error: 'No sheet data to save' };

    const sheetList = Object.entries(allSheets).map(([, sheet]: [string, ModifiedSheet]) => {
  ...
  const handleSaveProgress = async (newData: ModifiedSheet, refreshAll: boolean) => {
    ...
    const updatedSheets = allSheets
      ? { ...allSheets, [newData.sheetName]: newData }
      : { [newData.sheetName]: newData };
...
export const useGoogleDriveContext = (): GoogleDriveContextType => {
  ...
  if (!ctx) throw new Error('useGoogleDriveContext must be used inside GoogleDriveProvider');
  return ctx;
};
